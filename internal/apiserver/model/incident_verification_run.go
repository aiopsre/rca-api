...
type IncidentVerificationRunM struct {
	...
	JobID            *string `json:"job_id" gorm:"column:job_id;type:varchar(64);index:idx_incident_verification_runs_job_created,priority:1"`
	...
	// CreatedAt is also indexed by job_id for read-side run linkage queries.
	CreatedAt time.Time `json:"created_at" gorm:"column:created_at;type:datetime;index:idx_incident_verification_runs_incident_created,priority:2;index:idx_incident_verification_runs_job_created,priority:2;not null;default:CURRENT_TIMESTAMP"`
