...
type AIJobM struct {
	...
	IncidentID      string     `gorm:"column:incident_id;index:idx_ai_jobs_incident_id_created_at,priority:1;not null" json:"incident_id"`
	Pipeline        string     `gorm:"column:pipeline;not null" json:"pipeline"`
	Trigger         string     `gorm:"column:trigger;not null" json:"trigger"`
	Status          string     `gorm:"column:status;index:idx_ai_jobs_status_created_at,priority:1;not null" json:"status"`
