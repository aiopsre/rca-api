...
validate_incident_writeback_l2() {
	local body="$1"
	local diagnosis_raw="" evidence_refs_raw="" diagnosis_ev_count="" refs_ev_count=""
	local root_type="" confidence="" missing_count="" diagnosis_ev_first="" refs_ev_first=""
	...
	if ! need_cmd jq; then
		...
		if ! printf '%s' "${body}" | grep -Eq '"evidenceRefsJSON"|"evidence_refs_json"'; then
			fail_step "GetIncidentEvidenceRefsMissing" "${LAST_HTTP_CODE}" "${body}"
		fi
		if ! printf '%s' "${body}" | grep -q 'missing_evidence'; then
			fail_step "GetIncidentMissingEvidenceMissing" "${LAST_HTTP_CODE}" "${body}"
		fi
		if ! printf '%s' "${body}" | grep -Eq '"type"[[:space:]]*:[[:space:]]*"missing_evidence"'; then
			fail_step "GetIncidentRootTypeMissing" "${LAST_HTTP_CODE}" "${body}"
		fi
		return 0
	fi
	...
	if [[ -z "${diagnosis_raw}" ]] || [[ -z "${evidence_refs_raw}" ]]; then
	...
	if [[ "${root_type}" != "missing_evidence" ]]; then
	...
	missing_count="$(printf '%s' "${diagnosis_raw}" | jq -r '
		if (.missing_evidence | type) == "array" then
			[.missing_evidence[]?] | length
		else
			[(.hypotheses[]?.missing_evidence[]?)] | length
		end
	' 2>/dev/null || true)"
	...
	diagnosis_ev_count="$(printf '%s' "${diagnosis_raw}" | jq -r '[.root_cause.evidence_ids[]?] | length' 2>/dev/null || true)"
	if [[ -z "${diagnosis_ev_count}" ]] || (( diagnosis_ev_count < 1 )); then
		fail_step "DiagnosisEvidenceIDsEmpty" "${LAST_HTTP_CODE}" "${body}"
	fi

	refs_ev_count="$(printf '%s' "${evidence_refs_raw}" | jq -r '[.evidence_ids[]?] | length' 2>/dev/null || true)"
	if [[ -z "${refs_ev_count}" ]] || (( refs_ev_count < 1 )); then
		fail_step "EvidenceRefsIDsEmpty" "${LAST_HTTP_CODE}" "${body}"
	fi

	diagnosis_ev_first="$(printf '%s' "${diagnosis_raw}" | jq -r '.root_cause.evidence_ids[0] // empty' 2>/dev/null || true)"
	refs_ev_first="$(printf '%s' "${evidence_refs_raw}" | jq -r '.evidence_ids[0] // empty' 2>/dev/null || true)"
	if [[ -n "${diagnosis_ev_first}" ]] && [[ -n "${refs_ev_first}" ]] && [[ "${diagnosis_ev_first}" != "${refs_ev_first}" ]]; then
		fail_step "EvidenceIDsNotAligned" "${LAST_HTTP_CODE}" "${body}"
	fi

	EVIDENCE_ID="${diagnosis_ev_first:-${refs_ev_first}}"
}

rand="${RAND:-$RANDOM}"
now_epoch="$(date -u +%s)"
start_epoch="$((now_epoch - 1800))"
fingerprint="p0-l2-fp-${rand}"

ingest_body=$(cat <<EOF_INGEST
{"idempotencyKey":"idem-p0-l2-ingest-${rand}","fingerprint":"${fingerprint}","status":"firing","severity":"P1","service":"p0-l2-svc","cluster":"prod-l2","namespace":"default","workload":"p0-l2-workload","lastSeenAt":{"seconds":${now_epoch},"nanos":0},"labelsJSON":"{\"alertname\":\"LatencyHigh\",\"service\":\"p0-l2-svc\"}"}
EOF_INGEST
)
...
{"incidentID":"${INCIDENT_ID}","idempotencyKey":"idem-p0-l2-ai-run-${rand}","pipeline":"basic_rca","trigger":"manual","timeRangeStart":{"seconds":${start_epoch},"nanos":0},"timeRangeEnd":{"seconds":${now_epoch},"nanos":0},"inputHintsJSON":"{\"scenario\":\"L2\",\"force_no_evidence\":true,\"event_id\":\"${EVENT_ID:-unknown}\"}","createdBy":"system"}
EOF_RUN
)
...
validate_incident_writeback_l2 "${LAST_BODY}"
