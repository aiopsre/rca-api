...
SKILLSET_NAME="${SKILLSET_NAME:-script_executor_diagnosis_enrich}"
SKILL_ID="${SKILL_ID:-claude.diagnosis.script_enricher}"
SKILL_VERSION="${SKILL_VERSION:-1.0.0}"
SKILL_CAPABILITY="${SKILL_CAPABILITY:-diagnosis.enrich}"
INSTANCE_ID="${INSTANCE_ID:-script-executor-smoke-$$}"
...
BUNDLE_DIR="${BUNDLE_DIR:-/opt/workspace/study/rca-api/tools/ai-orchestrator/skill-bundles/diagnosis-script-enrich}"
...
if [[ -z "${AGENT_MODEL}" || -z "${AGENT_BASE_URL}" || -z "${AGENT_API_KEY}" ]]; then
  fail_step "ScriptExecutorConfig" "AGENT_MODEL, AGENT_BASE_URL, and AGENT_API_KEY are required"
fi
...
assert_json_expr "ResolveSkillsets" "${SKILLSET_RESOLVE_RESPONSE}" '
  any((.data.skillsets // .skillsets // [])[]?.skills[]?;
    ...
    and (.executorMode // .executor_mode // "") == "script"
  )
'

start_orchestrator

INCIDENT_CREATE_BODY='{"namespace":"default","workloadKind":"Deployment","workloadName":"script-executor-smoke","service":"script-executor-svc","severity":"P1"}'
...
RUN_BODY="$(cat <<JSON
{
  ...
  "idempotencyKey": "idem-script-executor-${INSTANCE_ID}",
  "pipeline": "${PIPELINE_ID}",
  "trigger": "manual",
  "timeRangeStart": {"seconds": ${START_EPOCH}, "nanos": 0},
  "timeRangeEnd": {"seconds": ${NOW_EPOCH}, "nanos": 0},
  "inputHintsJSON": "{\"scenario\":\"script_executor_diagnosis_enrich_smoke\"}",
  "createdBy": "system"
}
JSON
)"
...
assert_json_expr "SessionSkillMarker" "${SESSION_RESPONSE}" '
  (.data.context_state.skills.diagnosis_script_enrich.applied // .context_state.skills.diagnosis_script_enrich.applied) == true
'
...
'
assert_json_expr "SkillExecuteObservation" "${TOOLCALLS_RESPONSE}" '
  any((.data.toolCalls // .toolCalls // [])[]?;
    (.toolName // .tool_name // "") == "skill.execute"
    and (
...
REPORT_PATH="${WORKDIR}/script_executor_diagnosis_enrich_smoke_report.json"
...
PY
  ...
  "report": "script executor diagnosis.enrich smoke passed"
}
JSON

echo "PASS script executor diagnosis.enrich smoke"
