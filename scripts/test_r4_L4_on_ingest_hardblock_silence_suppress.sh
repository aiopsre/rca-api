...
start_server_or_fail() {
	...
	(
		cd "${REPO_ROOT}" && \
			bash -lc "${SERVER_CMD_BASE} --http.addr=127.0.0.1:${PORT} --redis.enabled=false --alerting.ingest_policy.dedup_window_seconds=300"
	) >"${SERVER_LOG}" 2>&1 &
	SERVER_PID="$!"
...
activate_policy_or_fail() {
	...
	call_or_fail "Policy.Activate" POST "${BASE_URL}/v1/alerting-policies/${policy_id}/activate" '{"operator":"script:r4_l4"}'
}
...
{"name":"r4-l4-on-ingest","description":"On-ingest hardblock regression policy","config":{"version":1,"defaults":{"on_ingest":{"enabled":false},"on_escalation":{"enabled":false},"scheduled":{"enabled":false}},"triggers":{"on_ingest":{"rules":[{"name":"on-ingest-run","match":{"alert_name":"r4-l4-on-ingest"},"action":{"run":true,"pipeline":"basic_rca","window_seconds":1800}}]}}}}
JSON
)"
