...
MOCK_PROM_PORT="${MOCK_PROM_PORT:-19096}"
...
fail_step() {
	...
	echo "recommended_step_index=${RECOMMENDED_STEP_INDEX:-NONE}"
	exit 1
}
...
if __name__ == "__main__":
    server = HTTPServer(("127.0.0.1", int(__import__("os").environ.get("T6_MOCK_PROM_PORT", "19096"))), Handler)
    server.serve_forever()
PYEOF

	T6_MOCK_PROM_PORT="${MOCK_PROM_PORT}" python3 "${MOCK_PROM_FILE}" >/dev/null 2>&1 &
	MOCK_PROM_PID=$!
...
assert_incident_playbook_and_plan() {
	local body="$1"
	if need_cmd jq; then
		local diagnosis_raw version items_count step_count
		...
		PLAYBOOK_JSON="$(printf '%s' "${diagnosis_raw}" | jq -c '.playbook // empty' 2>/dev/null || true)"
		if [[ -z "${PLAYBOOK_JSON}" ]]; then
			fail_step "GetIncident.PlaybookMissing" "${LAST_HTTP_CODE}" "${body}"
		fi
		version="$(printf '%s' "${PLAYBOOK_JSON}" | jq -r '.version // empty' 2>/dev/null || true)"
		if [[ "${version}" != "t6" ]]; then
			fail_step "GetIncident.PlaybookVersionInvalid" "${LAST_HTTP_CODE}" "${PLAYBOOK_JSON}"
		fi
		items_count="$(printf '%s' "${PLAYBOOK_JSON}" | jq -r '(.items // []) | length' 2>/dev/null || true)"
		if [[ -z "${items_count}" ]] || (( items_count < 1 || items_count > 10 )); then
			fail_step "GetIncident.PlaybookItemsInvalid" "${LAST_HTTP_CODE}" "${PLAYBOOK_JSON}"
		fi

		if ! printf '%s' "${PLAYBOOK_JSON}" | jq -e '
			(.items // [])
			| length > 0
			and all(.[];
				((.title // "") | length > 0)
				...
				and ((.steps // []) | length > 0)
				and all((.steps // [])[]; ((.text // "") | length > 0 and (length <= 256)))
				and ((.verification.use_verification_plan // false) == true)
				and ((.verification.recommended_steps // []) | length > 0)
				and ((.verification.expected_outcome // "") | length <= 256)
			)
		' >/dev/null 2>&1; then
			fail_step "GetIncident.PlaybookStructureInvalid" "${LAST_HTTP_CODE}" "${PLAYBOOK_JSON}"
		fi

		assert_no_sensitive "GetIncident.PlaybookNoSensitive" "${PLAYBOOK_JSON}"

		RECOMMENDED_STEP_INDEX="$(printf '%s' "${PLAYBOOK_JSON}" | jq -r '.items[0].verification.recommended_steps[0] // empty' 2>/dev/null || true)"
		if [[ -z "${RECOMMENDED_STEP_INDEX}" ]] || ! [[ "${RECOMMENDED_STEP_INDEX}" =~ ^[0-9]+$ ]]; then
			fail_step "GetIncident.RecommendedStepIndexMissing" "${LAST_HTTP_CODE}" "${PLAYBOOK_JSON}"
		fi
		if (( RECOMMENDED_STEP_INDEX >= step_count )); then
			fail_step "GetIncident.RecommendedStepIndexOutOfRange" "${LAST_HTTP_CODE}" "${PLAYBOOK_JSON}"
		fi

		STEP_TOOL="$(printf '%s' "${PLAN_JSON}" | jq -r --argjson idx "${RECOMMENDED_STEP_INDEX}" '.steps[$idx].tool // empty' 2>/dev/null || true)"
		STEP_PARAMS_JSON="$(printf '%s' "${PLAN_JSON}" | jq -c --argjson idx "${RECOMMENDED_STEP_INDEX}" '.steps[$idx].params // {}' 2>/dev/null || true)"
		if [[ -z "${STEP_TOOL}" || -z "${STEP_PARAMS_JSON}" ]]; then
			fail_step "GetIncident.RecommendedStepResolveFailed" "${LAST_HTTP_CODE}" "${PLAN_JSON}"
		fi
	else
		if ! printf '%s' "${body}" | grep -Eq '"playbook"'; then
			fail_step "GetIncident.PlaybookMissingNoJQ" "${LAST_HTTP_CODE}" "${body}"
		fi
		RECOMMENDED_STEP_INDEX="0"
		STEP_TOOL="mcp.query_metrics"
		STEP_PARAMS_JSON="{}"
	fi
}

assert_playbook_mirrored_to_toolcall() {
	local body="$1"
	if need_cmd jq; then
		local hit
		hit="$(printf '%s' "${body}" | jq -c '
			...
			| map(
				...
				| {
					...
					version: ($resp.playbook.version // "")
				  }
			)
			| map(select(.version == "t6"))
			| .[0] // empty
		' 2>/dev/null || true)"
		if [[ -z "${hit}" ]]; then
			fail_step "ListToolCalls.PlaybookMirrorMissing" "${LAST_HTTP_CODE}" "${body}"
		fi
		TOOL_CALL_ID="$(printf '%s' "${hit}" | jq -r '.tool_call_id // empty' 2>/dev/null || true)"
	else
		if ! printf '%s' "${body}" | grep -Eq '"playbook"'; then
			fail_step "ListToolCalls.PlaybookMirrorMissingNoJQ" "${LAST_HTTP_CODE}" "${body}"
		fi
	fi
...
evaluate_expectation_best_effort() {
	...
	expected_type="$(printf '%s' "${plan_json}" | jq -r --argjson i "${idx}" '.steps[$i].expected.type // "exists"' 2>/dev/null || true)"
	keyword="$(printf '%s' "${plan_json}" | jq -r --argjson i "${idx}" '.steps[$i].expected.keyword // empty' 2>/dev/null || true)"
	threshold="$(printf '%s' "${plan_json}" | jq -r --argjson i "${idx}" '.steps[$i].expected.value // empty' 2>/dev/null || true)"
...
namespace="t6-l1-ns-${rand}"
service="t6-l1-svc"
fingerprint="t6-l1-fp-${rand}"

create_ds_body=$(cat <<EOF2
{"type":"prometheus","name":"t6-mock-prom-${rand}","baseURL":"http://127.0.0.1:${MOCK_PROM_PORT}","authType":"none","timeoutMs":3000,"isEnabled":true}
EOF2
)
...
ingest_body=$(cat <<EOF2
{"idempotencyKey":"idem-t6-l1-ingest-${rand}","fingerprint":"${fingerprint}","status":"firing","severity":"P1","service":"${service}","cluster":"prod-t6","namespace":"${namespace}","workload":"t6-l1-workload","lastSeenAt":{"seconds":${now_epoch},"nanos":0},"labelsJSON":"{\"alertname\":\"T6Playbook\",\"service\":\"${service}\"}"}
EOF2
)
...
run_body=$(cat <<EOF2
{"incidentID":"${INCIDENT_ID}","idempotencyKey":"idem-t6-l1-ai-run-${rand}","pipeline":"basic_rca","trigger":"manual","timeRangeStart":{"seconds":${start_epoch},"nanos":0},"timeRangeEnd":{"seconds":${now_epoch},"nanos":0},"inputHintsJSON":"{\"scenario\":\"T6_L1_PLAYBOOK\"}","createdBy":"system"}
EOF2
)
...
assert_incident_playbook_and_plan "${LAST_BODY}"

call_or_fail "ListToolCalls" GET "${BASE_URL}/v1/ai/jobs/${JOB_ID}/tool-calls?offset=0&limit=50"
assert_playbook_mirrored_to_toolcall "${LAST_BODY}"
...
evaluate_expectation_best_effort "${PLAN_JSON}" "${RECOMMENDED_STEP_INDEX}" "${LAST_BODY}"

echo "PASS T6-L1"
echo "incident_id=${INCIDENT_ID}"
echo "job_id=${JOB_ID}"
echo "tool_call_id=${TOOL_CALL_ID}"
echo "recommended_step_index=${RECOMMENDED_STEP_INDEX}"
echo "playbook_version=t6"
