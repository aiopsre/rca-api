...
class ToolsetRegistryPhaseHRunnerTest(unittest.TestCase):
    @staticmethod
    ...
    def test_no_local_config_uses_server_resolve_and_builds_invoker(self) -> None:
        ...
        def _mcp_call(self, *, tool: str, input_payload: dict[str, object] | None, idempotency_key: str | None = None) -> dict[str, object]:
            ...
            return {"output": {"tool": tool, "payload": input_payload or {}, "idempotency_key": idempotency_key}}

        class _FakeClient:
            def __init__(self) -> None:
                self.resolve_toolset_calls = 0

            @staticmethod
            def get_job(job_id: str) -> dict[str, str]:
                return {"jobID": job_id, "incidentID": "inc-h1", "pipeline": "basic_rca"}

            def resolve_toolset(self, pipeline: str) -> dict[str, object]:
                self.resolve_toolset_calls += 1
                self.resolved_pipeline = pipeline
                return {
                    "pipeline": "basic_rca",
                    "toolset": {
                        "toolsetID": "server_default",
                        "providers": [
                            {
                                "type": "mcp_http",
        ...
        class _FakeCompiledGraph:
            def invoke(self, state: object) -> dict[str, object]:
                self.state = state
                return {"job_id": "job-h1", "started": True, "finalized": True}
        ...
        with mock.patch("orchestrator.tooling.invoker.MCPHttpProvider.call", new=_mcp_call), mock.patch.object(
            runner_module, "_new_client", return_value=client
            ...
            self.assertEqual(client.resolve_toolset_calls, 1)
    ...
    def test_server_resolve_404_fail_fast_without_graph_invoke(self) -> None:
        ...
        class _FakeClient:
            ...
            @staticmethod
            def get_job(job_id: str) -> dict[str, str]:
                return {"jobID": job_id, "incidentID": "inc-h2", "pipeline": "basic_rca"}

            def resolve_toolset(self, pipeline: str) -> dict[str, object]:
                self.resolve_toolset_calls += 1
                raise RuntimeError(f"resolve failed: not found pipeline={pipeline}")
        ...
        def _unexpected_builder(*_: object) -> object:
            class _Graph:
                def invoke(self, _state: object) -> dict[str, object]:
                    ...
                    return {"job_id": "job-h2", "started": True}

            return _Graph()

        client = _FakeClient()
        with mock.patch.object(runner_module, "_new_client", return_value=client), mock.patch.object(
