...
class ClaimStartResponseTest(unittest.TestCase):
    ...
    def test_has_verification_template_returns_false_for_missing_field(self) -> None:
        ...
        self.assertFalse(response.has_verification_template())
        self.assertIsNone(response.parse_verification_template())

    def test_parse_verification_template_from_response(self) -> None:
        from orchestrator.sdk.runtime_contract import ClaimStartResponse
        ...
        templates = [
            {
                ...
                "match": {"root_cause_types": ["deploy_regression"]},
                "steps": {"version": "v1", "steps": [{"id": "step-1", "tool": "mcp.query_metrics"}]},
            }
        ]

        response = ClaimStartResponse.from_api_response({
            "verificationTemplateJSON": json.dumps(templates),
        })

        self.assertTrue(response.has_verification_template())
        parsed = response.parse_verification_template()
        self.assertIsNotNone(parsed)
        self.assertEqual(len(parsed), 1)
        self.assertEqual(parsed[0]["name"], "deploy-regression")

    def test_parse_verification_template_returns_none_for_invalid_json(self) -> None:
        from orchestrator.sdk.runtime_contract import ClaimStartResponse

        response = ClaimStartResponse.from_api_response({
            "verificationTemplateJSON": "not valid json",
        })

        # has_verification_template returns True because the string is non-empty
        self.assertTrue(response.has_verification_template())
        # but parse returns None because JSON is invalid
        self.assertIsNone(response.parse_verification_template())
...
class VerificationTemplateMatchTest(unittest.TestCase):
    def test_match_by_root_cause_type(self) -> None:
        ...
        templates = [
            {
                "id": 1,
                "name": "deploy-regression",
                "match": {"root_cause_types": ["deploy_regression", "dependency_failure"]},
                "steps": {"version": "v1", "steps": [{"id": "step-1", "tool": "mcp.query_metrics"}]},
            },
            {
                "id": 2,
                "name": "latency-template",
                "match": {"root_cause_types": ["latency_spike"]},
                "steps": {"version": "v1", "steps": [{"id": "step-2", "tool": "mcp.query_logs"}]},
            },
        ]

        matched = match_verification_template(
            verification_templates=templates,
            root_cause_type="deploy_regression",
            patterns=[],
            confidence=0.8,
        )
        ...
        self.assertEqual(len(matched["steps"]), 1)
        ...
        templates = [
            {
                ...
                "match": {"patterns_contain": ["latency_spike", "5xx_increase"]},
                "steps": {"version": "v1", "steps": [{"id": "step-1", "tool": "mcp.query_metrics"}]},
            },
        ]
        ...
        templates = [
            {
                ...
                "match": {"root_cause_types": ["deploy_regression"], "confidence_min": 0.7},
                "steps": {"version": "v1", "steps": [{"id": "step-1", "tool": "mcp.query_metrics"}]},
            },
        ]
