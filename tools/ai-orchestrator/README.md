...
- `DEBUG`：默认 `0`
- `SKILLS_EXECUTION_MODE`：默认 `catalog`；`prompt_first` 时启用首条 Agent 驱动的 `diagnosis.enrich` 链路
...
- 当前 prompt-first Skills 只打通了 `diagnosis.enrich`，且只支持 prompt-only Skill，不支持 agent tool-calling。
